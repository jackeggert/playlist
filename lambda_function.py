...
if __name__=="__main__":
    ...
    if artist:
        ...
        print(artist[type_])
        for index in range(len(artist[type_])):
            client = boto3.resource('dynamodb')
            table = client.Table('artistsDropping')
            releaseDate = artist[type_][index]["date"]
            artistName = artist[type_][index]["artist"]
            queryVariable = artistName.replace(" ", "_")
            title = artist[type_][index]["release_name"]
            releaseType = 'ep'

            if len(releaseDate.split()) != 2 or not releaseDate.split()[1].isdigit():
                print("Invalid input date format. Please use the format 'Month DD'.")
            else:
                # Get the current date
                current_date = datetime.datetime.now().date()

                # Get the input date as a datetime object with the current year
                input_datetime = datetime.datetime.strptime(releaseDate + " " + str(datetime.datetime.now().year), "%B %d %Y")

                # Get the input date as a date object
                input_date = input_datetime.date()

                # Check if the input date is within the next week
                if input_date < current_date or input_date > current_date + timedelta(days=7):
                    print("Input date must be within the next week.")
                else:
                    # Format the input date into "YYYY-MM-DD" format
                    formatted_date = input_date.strftime("%Y-%m-%d")

                    input = {'artist': artistName, 'title': title, 'release_date': formatted_date, 'type': releaseType}
